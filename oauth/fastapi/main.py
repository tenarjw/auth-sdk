...
# Endpointy dla zarządzania bazą danych
@app.post("/db/create")
async def create_database(db: AsyncSession = Depends(get_db)):
    dm = DataManager()
    await dm.create(db)
...
async def insert_demo_data(db: AsyncSession = Depends(get_db)):
    dm = DataManager()
    address_id = await dm.add_address(country='Poland', session=db)
    # Zwróć uwagę, że tutaj nie ma już await dm.create(db).
    # Tworzenie tabel odbywa się raz, na starcie aplikacji.
    await dm.add_user('demo', 'demo', name='John Down', email='jd@example.com', session=db)
    await dm.add_client(ident='demoapp', secret='secret', system_user_id=1, auth_redirect_uri='http://127.0.0.1:3000', session=db)
    return {"message": "Demo data inserted successfully"}
@app.get("/db/test")
async def test_data(db: AsyncSession = Depends(get_db)):
    dm = DataManager()
    user_id = await dm.check_user('demo', 'demo', session=db)
    client = await dm.get_client(1, session=db)
    return {
        "user_id": user_id,
...
async def get_client_uuid(id: int = Query(..., description="Client ID"), db: AsyncSession = Depends(get_db)):
    dm = DataManager()
    client = await dm.get_client(id, session=db)
    if client:
...
async def add_user(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    dm = DataManager()
    user_id = await dm.add_user(user.ident, user.secret, name=user.name, email=user.email, session=db)
...
async def add_client(client: ClientCreate, db: AsyncSession = Depends(get_db)):
    ...
    dm = DataManager()
    client_id = await dm.add_client(client.ident, client.secret, client.user_id, client.uri, session=db)
...
async def login(
    ...
    dm = DataManager()
    user_id = await dm.check_user(username, password, session=db)
    if user_id:
